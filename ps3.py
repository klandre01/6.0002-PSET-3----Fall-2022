...
class Room(object):
    ...
    def __init__(self, width, height, dust_amount):
        ...
        self.width = width
        self.height = height
        # dust[w][h] returns dust on tile (w,h)
        self.dust = [[dust_amount for h in range(height)] for w in range(width)]
    ...
    def clean_tile_at_position(self, pos, cleaning_volume):
        ...
        dust = self.get_dust_amount(w, h)
        if cleaning_volume > dust:
            self.dust[w][h] = 0
        else:
            self.dust[w][h] -= cleaning_volume
    ...
    def is_position_in_room(self, pos):
        ...
        w = pos.get_x()
        h = pos.get_y()
        return (w < self.width and w >= 0 and h < self.height and h >= 0)
...
class NormalRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_pos = position.get_new_position(self.direction, self.speed)
        room = self.room
        if room.is_position_in_room(new_pos):
            self.set_position(new_pos)
            room.clean_tile_at_position(new_pos, self.cleaning_volume)
        else:
            self.direction = random.random() * 360
...
class ClumsyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        speed = self.speed
        vol = self.cleaning_volume
        # drop dust
        if self.does_drop_dust():
            dropped_dust = random.random() * 0.5
            room.clean_tile_at_position(pos, 0 - dropped_dust)
        # moving to next tile
        new_pos = pos.get_new_position(angle, speed)
        if room.is_position_in_room(new_pos):
            self.set_position(new_pos)
            room.clean_tile_at_position(new_pos, vol)
        else:
            self.direction = random.random() * 360
...
class SensingRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        dust_dict = self.scan_surrounding_area()
        amounts = list(dust_dict.values())
        max_dust = max(amounts) # maximum amount of dust
        # angles with max amount of dust
        angles = [angle for angle in list(dust_dict.keys()) if dust_dict[angle] == max_dust]
        dir = random.choice(angles) # the direction we will move in
...
def run_simulation(num_robots, speed, cleaning_volume, width, height, dust_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials): # for each trial
        room = Room(width, height, dust_amount) # new room
        robo_list = [robot_type(room, speed, cleaning_volume) for robo in range(num_robots)] # robots in the room
        while room.get_num_cleaned_tiles() < room.get_num_tiles() * min_coverage: # room isn't clean enough
